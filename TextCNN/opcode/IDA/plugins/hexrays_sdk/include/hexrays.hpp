 ...
 *      many auxiliary classes, please see their definitions below.
 *
 */
 ...
 *      where the left operand is 'r0', its size is 4 bytes (r0.4)
 *            the right operand is constant '8', its size is 4 bytes (#8.4)
...
#define   VLR_UNION  0x07 //   union of sub-ranges
                          ...
                          //   intersection
  int size;               // operand size: 1,2,4,8 bytes
  ...
  union
  {
    ...
    {                     // values that are between VALUE and LIMIT
      ...
      uvlr_t value;       // the main/initial value
      uvlr_t limit;       // the final value
...
struct optinsn_t
{
  ...
  ///            sub-instructions may not be converted int nop but
...
/// \param opt an instance of optinsn_t. can not be destroyed before calling
...
/// \param opt an instance of optblock_t. can not be destroyed before calling
...
/// If this function returns true, the opcode can not appear in the middle
...
bool hexapi must_mcode_close_block(mcode_t mcode, bool including_calls);


/// May opcode be propagates?
...
bool hexapi is_mcode_propagatable(mcode_t mcode);
...
// If the opcode can not be negated, return m_nop
...
// If the opcode can not be swapped, return m_nop
...
// If the opcode can not be transformed into signed form, simply return it.
...
// If the opcode can not be transformed into unsigned form, simply return it.
...
public:
  ...
  bool hexapi is_aliasable(const mbl_array_t *mb, int size) const;
};
...
#define CVAR_FAKE    0x00000200 ///< fake return variable
...
#define CVAR_REGNAME 0x00020000 ///< has a register name (like _RAX)
#define CVAR_NOPTR   0x00040000 ///< variable can not be a pointer (user choice)
//@}
...
#define SVW_SOFT  0x02 // may fail and return false;
  ...
  /// \param mba ptr to the current mbl_array_t
  /// Aliasable variables may be modified indirectly (through a pointer)
  bool is_aliasable(const mbl_array_t *mba) const
  {
...
public:
  ...
  DECLARE_COMPARISON_OPERATORS(ivl_tpl)
  int compare(const ivl_tpl &r) const;
  DEFINE_MEMORY_ALLOCATION_FUNCS()
};

//-------------------------------------------------------------------------
struct ivl_t : public ivl_tpl<uval_t> // an interval
{
private:
...
public:
  ...
  T count(void) const; // sum of the interval sizes
  ...
  bool is_subset_of(const ivlset_tpl &ivs) const { return ivs.includes(*this); }
  bool single_value(T v) const { return nivls() == 1 && bag[0].off == v && bag[0].size == 1; }

  DECLARE_COMPARISON_OPERATORS(ivlset_tpl)
...
struct ivlset_t : public ivlset_tpl<ivl_t, uval_t>
{
...
// in many our algorthims. This functionality is available for plugins too.
...
/// See mbl_array_t::for_all_topinsns, minsn_t::for_all_insns,
///     mblock_::for_all_insns, mbl_array_t::for_all_insns
...
struct lvar_ref_t
{
  ...
  /// the referenced variable, we keep a pointer to mbl_array_t here.
  /// Note: this means this class and consequently mop_t, minsn_t, mblock_t
  ///       are specific to a mbl_array_t object and can not migrate between
...
// Please note there are some exceptions: udts, floating point, xmm/ymm, etc
...
/// Later operands will be used in instructions, which are grouped into blocks.
/// An array of basic blocks + some additional info will be the microcode.
class mop_t
{
...
#define OPROP_UDEFVAL 0x10 ///< uses undefined value
  ...
  /// \param loc location
  /// \param fullsize mba->fullsize
  /// Note: this function can not handle scattered locations.
  /// \return success
  ...
  /// \param include_ldx consider ldx as having side effects?
...
class graph_chains_t : public qvector<block_chains_t>
{
...
#define EQ_OPTINSN 0x0008      ///< optimize mop_d operands
  ...
  /// Is an unknown call?
  /// Unknown calls are resolved by mbl_array_t::analyze_calls()
  /// They exist until the MMAT_CALLS maturity level.
  bool is_unknown_call(void) const { return is_mcode_call(opcode) && d.t == mop_z; }
  ...
  funcrole_t get_role(void) const { return d.t == mop_f ? d.f->role : ROLE_UNK; }
...
/// All blocks are part of a doubly linked list as well as can be addressed
/// by indexing the mba->natural array. A block contains a doubly linked list
/// of instructions, various localtion lists that are used for data flow
...
#define MBL_VALRANGES   0x8000  ///< should optimize using value ranges
  ...
  ea_t end;                     ///< end address
                                ///< note: we can not rely on start/end addresses
  ...
  mbl_array_t *mba;             ///< the parent micro block array
  ...
  /// The removed iinstruction is not deleted, the caller gets its ownership
  ...
  /// See also mbl_array_t::for_all_topinsns()
  int hexapi for_all_insns(minsn_visitor_t &mv);
  ...
  /// to nop's). See also minsn_t::optimize_solo()
  ...
  /// memory. On the other hand, we can not tell for sure which memory cells
  ...
  ///             i2 is excluded from the range. it can be specified as NULL.
  ///             i1 and i2 must belong to the same block.
  bool hexapi is_rhs_redefined(minsn_t *ins, minsn_t *i1, minsn_t *i2);
...
#define FD_DIRTY    0x0004  ///< ignore possible implicit definitions
  ...
  //@{
#define VR_AT_START 0x0000    ///< get value ranges at the block start
#define VR_AT_END   0x0001    ///< get value ranges at the block end, just before
...
//-------------------------------------------------------------------------
/// Array of micro blocks represents microcode for a decompiled function.
...
public:
  ...
  bool may_del_asserts(void) const { return (flags & MBA_DELASRT) != 0; }
  ...
  bool has_overvars(void)   const { return (flags & MBA_OVERVAR) != 0; }
...
#define ACFL_GUESS   0x10 ///< may guess calling conventions
  ...
  /// \param bblk he new block will be inserted before BBLK
  ...
  void hexapi serialize(bytevec_t &b) const;
  ...
  static mbl_array_t *hexapi deserialize(const uchar *bytes, size_t nbytes);
...
{
public:
  mbl_array_t *mba;     // ptr to mbl array
...
inline bool mop_t::has_side_effects(bool include_ldx) const
{
  return is_insn() && d->has_side_effects(include_ldx);
}
...
inline const ivl_t &mbl_array_t::get_std_region(memreg_index_t idx) const
{
  return std_ivls[idx].ivl;
}

inline const ivl_t &mbl_array_t::get_lvars_region(void) const
{
  return get_std_region(MMIDX_LVARS);
}

inline const ivl_t &mbl_array_t::get_shadow_region(void) const
{
  return get_std_region(MMIDX_SHADOW);
}

inline const ivl_t &mbl_array_t::get_args_region(void) const
{
  return get_std_region(MMIDX_ARGS);
}

inline ivl_t mbl_array_t::get_stack_region(void) const
{
...
#define VDRUN_LUMINA  0x0080  ///< use lumina server
//@}
...
#define GCO_DEF 0x0004  ///< is destination operand?
  ...
  bool hexapi append_to_list(mlist_t *list, const mbl_array_t *mba) const;
};
...
struct bit_bound_t
{
  int16 nbits; // total number of non-zero bits. we can not guarantee that
  ...
  int16 sbits; // number of sign bits (they can be either 0 or 1, all of them)
               ...
               // if bits are unknown and can be anything, they can not be included
               // in sbits.
...
inline cfuncptr_t decompile_func(func_t *pfn, hexrays_failure_t *hf, int flags=0)
{
...
//@}
  ...
  bool __fastcall hexapi ui_set_call_type(const cexpr_t *e);
  ...
  /// \return false if failed
  /// \param v pointer to local variable
  bool __fastcall hexapi set_noptr_lvar(lvar_t *v);
  ...
  /// some function lines can not have comments. This function checks if a
...
enum hexcall_t
{
  ...
  hx_mbl_array_t_idaloc2vd,
  hx_mbl_array_t_vd2idaloc,
  hx_mbl_array_t_term,
  hx_mbl_array_t_optimize_local,
  hx_mbl_array_t_build_graph,
  hx_mbl_array_t_get_graph,
  hx_mbl_array_t_analyze_calls,
  hx_mbl_array_t_optimize_global,
  hx_mbl_array_t_alloc_lvars,
  hx_mbl_array_t_dump,
  hx_mbl_array_t_vdump_mba,
  hx_mbl_array_t_print,
  hx_mbl_array_t_verify,
  hx_mbl_array_t_mark_chains_dirty,
  hx_mbl_array_t_insert_block,
  hx_mbl_array_t_remove_block,
  hx_mbl_array_t_remove_empty_blocks,
  hx_mbl_array_t_combine_blocks,
  hx_mbl_array_t_for_all_ops,
  hx_mbl_array_t_for_all_insns,
  hx_mbl_array_t_for_all_topinsns,
  hx_mbl_array_t_find_mop,
  hx_mbl_array_t_arg,
  hx_mbl_array_t_serialize,
  hx_mbl_array_t_deserialize,
...
//--------------------------------------------------------------------------
inline ea_t get_merror_desc(qstring *out, merror_t code, mbl_array_t *mba)
{
  ea_t retval;
...
//--------------------------------------------------------------------------
inline bool vdloc_t::is_aliasable(const mbl_array_t *mb, int size) const
{
...
//--------------------------------------------------------------------------
inline int get_mreg_name(qstring *out, int bit, int width, void *ud)
{
  return (int)(size_t)hexdsp(hx_get_mreg_name, out, bit, width, ud);
}
...
//--------------------------------------------------------------------------
inline bool mop_t::create_from_mlist(mbl_array_t *mba, const mlist_t &lst, sval_t fullsize)
{
  return (uchar)(size_t)hexdsp(hx_mop_t_create_from_mlist, this, mba, &lst, fullsize) != 0;
}

//--------------------------------------------------------------------------
inline bool mop_t::create_from_ivlset(mbl_array_t *mba, const ivlset_t &ivs, sval_t fullsize)
{
  return (uchar)(size_t)hexdsp(hx_mop_t_create_from_ivlset, this, mba, &ivs, fullsize) != 0;
}

//--------------------------------------------------------------------------
inline void mop_t::create_from_vdloc(mbl_array_t *mba, const vdloc_t &loc, int _size)
{
  hexdsp(hx_mop_t_create_from_vdloc, this, mba, &loc, _size);
}

//--------------------------------------------------------------------------
inline void mop_t::create_from_scattered_vdloc(mbl_array_t *mba, const char *name, tinfo_t type, const vdloc_t &loc)
{
...
//--------------------------------------------------------------------------
inline int minsn_t::optimize_subtree(mblock_t *blk, minsn_t *top, minsn_t *parent, minsn_t **converted_call, int optflags)
{
...
//--------------------------------------------------------------------------
inline bool minsn_t::has_side_effects(bool include_ldx) const
{
  return (uchar)(size_t)hexdsp(hx_minsn_t_has_side_effects, this, include_ldx) != 0;
}
...
//--------------------------------------------------------------------------
inline bool mblock_t::is_rhs_redefined(minsn_t *ins, minsn_t *i1, minsn_t *i2)
{
...
//--------------------------------------------------------------------------
inline vdloc_t mbl_array_t::idaloc2vd(const argloc_t &loc, int width, sval_t spd)
{
  vdloc_t retval;
  hexdsp(hx_mbl_array_t_idaloc2vd, &retval, &loc, width, spd);
  return retval;
}

//--------------------------------------------------------------------------
inline argloc_t mbl_array_t::vd2idaloc(const vdloc_t &loc, int width, sval_t spd)
{
  argloc_t retval;
  hexdsp(hx_mbl_array_t_vd2idaloc, &retval, &loc, width, spd);
  return retval;
}

//--------------------------------------------------------------------------
inline void mbl_array_t::term(void)
{
  hexdsp(hx_mbl_array_t_term, this);
}

//--------------------------------------------------------------------------
inline int mbl_array_t::optimize_local(int locopt_bits)
{
  return (int)(size_t)hexdsp(hx_mbl_array_t_optimize_local, this, locopt_bits);
}

//--------------------------------------------------------------------------
inline merror_t mbl_array_t::build_graph(void)
{
  return (merror_t)(size_t)hexdsp(hx_mbl_array_t_build_graph, this);
}

//--------------------------------------------------------------------------
inline mbl_graph_t *mbl_array_t::get_graph(void)
{
  return (mbl_graph_t *)hexdsp(hx_mbl_array_t_get_graph, this);
}

//--------------------------------------------------------------------------
inline int mbl_array_t::analyze_calls(int acflags)
{
  return (int)(size_t)hexdsp(hx_mbl_array_t_analyze_calls, this, acflags);
}

//--------------------------------------------------------------------------
inline merror_t mbl_array_t::optimize_global(void)
{
  return (merror_t)(size_t)hexdsp(hx_mbl_array_t_optimize_global, this);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::alloc_lvars(void)
{
  hexdsp(hx_mbl_array_t_alloc_lvars, this);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::dump(void) const
{
  hexdsp(hx_mbl_array_t_dump, this);
}

//--------------------------------------------------------------------------
inline AS_PRINTF(3, 0) void mbl_array_t::vdump_mba(bool _verify, const char *title, va_list va) const
{
  hexdsp(hx_mbl_array_t_vdump_mba, this, _verify, title, va);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::print(vd_printer_t &vp) const
{
  hexdsp(hx_mbl_array_t_print, this, &vp);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::verify(bool always) const
{
  hexdsp(hx_mbl_array_t_verify, this, always);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::mark_chains_dirty(void)
{
  hexdsp(hx_mbl_array_t_mark_chains_dirty, this);
}

//--------------------------------------------------------------------------
inline mblock_t *mbl_array_t::insert_block(int bblk)
{
  return (mblock_t *)hexdsp(hx_mbl_array_t_insert_block, this, bblk);
}

//--------------------------------------------------------------------------
inline bool mbl_array_t::remove_block(mblock_t *blk)
{
  return (uchar)(size_t)hexdsp(hx_mbl_array_t_remove_block, this, blk) != 0;
}

//--------------------------------------------------------------------------
inline bool mbl_array_t::remove_empty_blocks(void)
{
  return (uchar)(size_t)hexdsp(hx_mbl_array_t_remove_empty_blocks, this) != 0;
}

//--------------------------------------------------------------------------
inline bool mbl_array_t::combine_blocks(void)
{
  return (uchar)(size_t)hexdsp(hx_mbl_array_t_combine_blocks, this) != 0;
}

//--------------------------------------------------------------------------
inline int mbl_array_t::for_all_ops(mop_visitor_t &mv)
{
  return (int)(size_t)hexdsp(hx_mbl_array_t_for_all_ops, this, &mv);
}

//--------------------------------------------------------------------------
inline int mbl_array_t::for_all_insns(minsn_visitor_t &mv)
{
  return (int)(size_t)hexdsp(hx_mbl_array_t_for_all_insns, this, &mv);
}

//--------------------------------------------------------------------------
inline int mbl_array_t::for_all_topinsns(minsn_visitor_t &mv)
{
  return (int)(size_t)hexdsp(hx_mbl_array_t_for_all_topinsns, this, &mv);
}

//--------------------------------------------------------------------------
inline mop_t *mbl_array_t::find_mop(op_parent_info_t *ctx, ea_t ea, bool is_dest, const mlist_t &list)
{
  return (mop_t *)hexdsp(hx_mbl_array_t_find_mop, this, ctx, ea, is_dest, &list);
}

//--------------------------------------------------------------------------
inline lvar_t &mbl_array_t::arg(int n)
{
  return *(lvar_t *)hexdsp(hx_mbl_array_t_arg, this, n);
}

//--------------------------------------------------------------------------
inline void mbl_array_t::serialize(bytevec_t &b) const
{
  hexdsp(hx_mbl_array_t_serialize, this, &b);
}

//--------------------------------------------------------------------------
inline mbl_array_t *mbl_array_t::deserialize(const uchar *bytes, size_t nbytes)
{
  return (mbl_array_t *)hexdsp(hx_mbl_array_t_deserialize, bytes, nbytes);
}
...
//--------------------------------------------------------------------------
inline bool gco_info_t::append_to_list(mlist_t *list, const mbl_array_t *mba) const
{
...
//--------------------------------------------------------------------------
inline cfuncptr_t decompile(const mba_ranges_t &mbr, hexrays_failure_t *hf, int flags)
{
  return cfuncptr_t((cfunc_t *)hexdsp(hx_decompile, &mbr, hf, flags));
}

//--------------------------------------------------------------------------
inline mbl_array_t *gen_microcode(const mba_ranges_t &mbr, hexrays_failure_t *hf, const mlist_t *retlist, int flags, mba_maturity_t reqmat)
{
  return (mbl_array_t *)hexdsp(hx_gen_microcode, &mbr, hf, retlist, flags, reqmat);
}
...
//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ui_set_call_type(const cexpr_t *e)
{
...
//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_noptr_lvar(lvar_t *v)
{
